<?php
$page_title = "Editar Estado de Actividad";
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->prepare("SELECT * FROM estados_actividad WHERE id = ?");
    $stmt->execute([$id]);
    $estado = $stmt->fetch();
    if (!$estado) redirect('index.php');
} catch (PDOException $e) {
    setFlashMessage('error', 'Error al cargar el estado.');
    redirect('index.php');
}
?>
<div class="flex flex-col items-center justify-center min-h-[70vh]">
    <div class="bg-white rounded-2xl shadow-2xl p-8 border border-cadet/40 w-full max-w-lg mx-auto">
        <h1 class="text-2xl font-bold text-primary flex items-center gap-2 mb-6">
            <i class="fas fa-flag text-accent"></i> Editar Estado de Actividad
        </h1>
        <form action="update.php" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="nombre" name="nombre" required maxlength="100" value="<?php echo htmlspecialchars($estado['nombre']); ?>" class="w-full px-4 py-2 border border-cadet/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-white placeholder-cadet text-darkpurple shadow-sm" placeholder="Nombre del estado de actividad">
            </div>
