...
try {
    ...
    if (!$organizacion) redirect('index.php');
...
<div class="flex flex-col items-center justify-center min-h-[70vh]">
    <div class="bg-white rounded-2xl shadow-2xl p-8 border border-cadet/40 w-full max-w-lg mx-auto">
        <h1 class="text-2xl font-bold text-primary flex items-center gap-2 mb-6">
            <i class="fas fa-building text-accent"></i> Editar Organización
        </h1>
        <form action="update.php" method="POST" class="space-y-6">
            <input type="hidden" name="id" value="<?php echo $organizacion['id']; ?>">
            <div>
                <label for="nombre" class="block text-sm font-semibold text-darkpurple mb-1">Nombre de la Organización *</label>
                <input type="text" id="nombre" name="nombre" required maxlength="255" value="<?php echo htmlspecialchars($organizacion['nombre']); ?>" class="w-full px-4 py-2 border border-cadet/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-white placeholder-cadet text-darkpurple shadow-sm" placeholder="Nombre de la organización">
